...
# my octave MAT is not compatible
# DATA_PATH = "./data/test_/"
# SUB_PATH = 'Set*'
...
def get_img_list(data_path):
    l = glob.glob(os.path.join(data_path, "gt", "*"))
    train_list = []
    for f in l:
        f_base = os.path.basename(f)
        # print("base name: ", f_base)
        train_list.append([f, data_path + "/scale/" + f_base[:-4] + "_2.mat", 2])
        train_list.append([f, data_path + "/scale/" + f_base[:-4] + "_3.mat", 3])
        train_list.append([f, data_path + "/scale/" + f_base[:-4] + "_4.mat", 4])
    return train_list
...
def get_test_image(test_list, offset, batch_size):
    ...
    for pair in target_list:
        print("\nMAT files: ", pair[0], ", ", pair[1])

        # GT key:  __globals__
        # GT key:  patch
        # GT key:  __version__
        # GT key:  __header__
        # SCALE key:  __globals__
        # SCALE key:  patch
        # SCALE key:  __version__
        # SCALE key:  __header__
        # for key in scipy.io.loadmat(pair[0]):
        #     print("GT key: ", key)
        # for key in scipy.io.loadmat(pair[1]):
        #     print("SCALE key: ", key)

        gt_img = scipy.io.loadmat(pair[0])['patch']  # ['img_raw']
